...
pub fn spawn(tx: UnboundedSender<KeyEvent>) {
    crate::macos_key_tap::spawn(tx, crate::macos_key_tap::TapMode::Grab);
}
...
mod platform {
    ...
    pub fn spawn(tx: UnboundedSender<KeyEvent>) {
        std::thread::spawn(move || {
            eprintln!(
                "[listener] starting rdev::grab (needs Accessibility + Input Monitoring perms)"
            );
            ...
            match result {
                ...
                Ok(Err(e)) => eprintln!(
                    "[listener] rdev grab error: {e:?} — check System Settings → Privacy & Security → Accessibility + Input Monitoring"
                ),
                Err(payload) => {
